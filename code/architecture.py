...
class GCN2(torch.nn.Module):
    ...
    def forward(self, x_c, edge_index_c, edge_weight_c, x_a, edge_index_a, edge_weight_a, batch_index_c, batch_index_a, cond=None):
        # Conv layers
        hidden1 = self.conv1(x_c, edge_index_c, edge_weight_c).relu()
        hidden1 = self.dropout1(hidden1)
        hidden1 = self.conv2(hidden1, edge_index_c, edge_weight_c).relu()
        hidden1 = self.dropout1(hidden1)
        hidden1 = self.conv3(hidden1, edge_index_c, edge_weight_c).relu()
        hidden1 = self.dropout1(hidden1)
        hidden1 = self.conv4(hidden1, edge_index_c, edge_weight_c).relu()
        hidden1 = self.dropout1(hidden1)
        hidden1 = gap(hidden1, batch_index_c)

        hidden2 = self.conv1(x_a, edge_index_a, edge_weight_a).relu()
        hidden2 = self.dropout1(hidden2)
        hidden2 = self.conv2(hidden2, edge_index_a, edge_weight_a).relu()
        hidden2 = self.dropout1(hidden2)
        hidden2 = self.conv3(hidden2, edge_index_a, edge_weight_a).relu()
        hidden2 = self.dropout1(hidden2)
        hidden2 = self.conv4(hidden2, edge_index_a, edge_weight_a).relu()
        hidden2 = self.dropout1(hidden2)
...
class GCN2o(torch.nn.Module):
    def __init__(self, conv_function, input_channels, embedding_size, linear_size, add_params_num=0):
        # Init parent
        super(GCN2o, self).__init__()
        self.crafted_add_params_num = add_params_num
    ...
    def forward(self, x_c, edge_index_c, edge_weight_c, x_a, edge_index_a, edge_weight_a, batch_index_c, batch_index_a, cond=None):
        ...
        batch_index = torch.cat([batch_index_c, batch_index_a], dim=0)
        hidden = gap(hidden, batch_index)
